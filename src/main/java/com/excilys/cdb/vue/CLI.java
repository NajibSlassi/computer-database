...
public class CLI {
	...
	private void printMenu() {
		...
		System.out.println("2 - Lister les soci�t�s");
		System.out.println("3 - D�tail d'un ordinateur");
		System.out.println("4 - Cr�er un ordinateur");
		System.out.println("5 - Mettre � jour un ordinateur");
		System.out.println("6 - Supprimer un ordinateur");
		System.out.println("7 - Supprimer une soci�t�");
	}
	public void sendToService(int choice) throws ParseException {
		...
		case 2:
			...
			break;
		case 3:
			int consulter =readInt("Entrez l'id de l'ordinateur � consulter");
	...
	public DTOComputer updateComputer() throws ParseException {
		LOGGER.debug("Mise � jour de l'ordinateur...: ");
		System.out.println("Entrez l'id de l'ordinateur que vous voulez mettre � jour:");
		...
		System.out.println("Entrez le nouveau id de la soci�t� fabricante:");
		String companyId = sc.nextLine();
		LOGGER.info("nouveau id de la soci�t� fabricante: " + companyId);
	...
	public void showComputerDetails(DTOComputer computer) {
		LOGGER.debug("Affichage des d�tails d'un ordinateur...: ");
		...
		try {
			...
			LOGGER.warn("Aucun ordinateur correspondant trouv�");
	...
	public void quitProgram() {
		...
		int m = 0;
		do {
			System.out.println("Quitter? 0 = Oui, n'importe quel num�ro = Non");
			if (scan.hasNextInt()) {
				m = scan.nextInt();
				wenttocatch = true;
			} else {
	...
	public DTOComputer createComputer() throws ParseException {
		LOGGER.debug("cr�ation d'un nouveau ordinateur");
		...
		while (dateIntro.compareTo(dateDisc) > 0) {
			LOGGER.info("date de sortie inf�rieur � la date d'entr�e");
			System.out.println("Date d'entr� en stock: " + introduced
					+ " sup�rieure � la date de sortie que vous avez saisi, Veuillez entrez une date de sortie ult�rieure :");
			discontinued = sc.nextLine();
			dateDisc = dateFormat.parse(discontinued);
		}

		System.out.println("Entrez l'id de la soci�t� fabricante:");
		String companyId = sc.nextLine();
		LOGGER.info("id de la soci�t� fabricante saisi par l'utilisateur: " + companyId);
	...
	public void showCompanies(List<DTOCompany> c) {
		...
		LOGGER.debug("Affichage de la liste des soci�t�s...");
	...
	public int readPage() {
		System.out.println("Entrez la page � consulter");
		int id = sc.nextInt();
		LOGGER.info("Page demand� par l'utilisateur: " + id);
		return id;
	}

	public int readLimit() {
		System.out.println("Entrez le nombre d'�l�ments par page");
		int id = sc.nextInt();
		LOGGER.info("Elements par page deemand�s par l'utilisateur: " + id);
		return id;
	}
